...
async function handleEvents(events: LineEvent[]): Promise<void> {
  ...
  for (const event of events) {
    ...
    console.log("[webhook] faq loaded, rows:", faqRows.length);
    ...
    console.log("[webhook] replyText:", replyText.substring(0, 50));
...
EOFgit add .
git commit -m "fix: add detailed logs for debug"
git push origin main
cat > app/api/line-webhook/route.ts << 'EOF'
import { NextRequest, NextResponse } from "next/server";
import * as crypto from "crypto";
import { messagingApi } from "@line/bot-sdk";
import { getFaq, faqToText } from "@/lib/sheet";
